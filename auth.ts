...
// Two self-contained dev-only sign-ins so the full applicant + admin flow can be
// exercised locally without Google credentials. Never registered in production.
    ...
    async authorize() {
      const user = await prisma.user.upsert({
        ...
        update: {
          ...
          // The dev applicant is a throwaway test account: reset it to PENDING
          // on every login so the application flow can be re-tested from scratch.
      ...
      // For the test applicant, clear any prior application so ensureRegistrationRecords
      // opens a fresh draft on sign-in (admins don't get an application).
      if (!isAdmin) {
